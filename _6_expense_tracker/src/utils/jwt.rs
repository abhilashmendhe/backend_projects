...
pub fn create_token(secret: &str, username: String) -> Result<String, ExpenseTrackerErr> {
...
pub fn validate_token(secret: &str, token: &str) -> Result<bool, ExpenseTrackerErr> {
    decode::<Claims>(
        token,
    ...
    .map_err(|err| {
        ...
        match err.kind() {
            ...
            | jsonwebtoken::errors::ErrorKind::InvalidSignature => ExpenseTrackerErr::AppError(
                AppError::new(StatusCode::UNAUTHORIZED, "Not Authenticated."),
            ),
            _ => ExpenseTrackerErr::AppError(AppError::new(
                StatusCode::INTERNAL_SERVER_ERROR,
                "Server failed to validate token.",
            )),
        }
    })
    .map(|_| true)
