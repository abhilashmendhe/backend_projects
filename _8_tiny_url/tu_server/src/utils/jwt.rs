...
pub fn create_token(secret: &str, username: String) -> Result<String, TinyUrlError> {
...
pub fn validate_token(secret: &str, token: &str) -> Result<bool, TinyUrlError> {
    decode::<Claims>(
        token,
    ...
    .map_err(|err| {
        ...
        match err.kind() {
            ...
            | jsonwebtoken::errors::ErrorKind::InvalidSignature => TinyUrlError::AppError(
                AppError::new(StatusCode::UNAUTHORIZED, "Not Authenticated."),
            ),
            _ => TinyUrlError::AppError(AppError::new(
                StatusCode::INTERNAL_SERVER_ERROR,
